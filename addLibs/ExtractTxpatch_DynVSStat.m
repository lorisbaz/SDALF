        ...
        for part = 2:size(div,2) % NO head
            ...
            for p = 1:size(patches,1)
                if histo_entropy(order(order2(p)))>Thresh_entr  ...
                    ...
                    epitome.entr{ep}  = histo_entropy(order(order2(p)));
					epitome.w_ncc{ep} = area_ncc(order(order2(p)));
                    epitome.pos{ep}   = patches(order(order2(p)),:);
					epitome.thrCC{ep} = MNCC{order(order2(p))};
% 					epitome.areaCC{ep}= area_ncc(order(order2(p)));
					...
					data_e(:,ep) = [ctr';lbph']; % justapposition of data_e
			...
			if ~isempty(epitome.patch)
                [clust_res,centres]= MS_Clustering2(data_e',MSpar,'Uniform',[length(ctr),length(lbph)],2);
                K = max(clust_res);
            ...
            try
                for k = 1:length(rap)
                    if rap(k)>0
                        max_txpatch(ii,part).patch{ep}	= epitome.patch{rap(k)};
                        max_txpatch(ii,part).entr{ep}	= epitome.entr{rap(k)};
                        max_txpatch(ii,part).w_ncc{ep}	= epitome.w_ncc{rap(k)};
                        max_txpatch(ii,part).pos{ep}	= epitome.pos{rap(k)};
                        max_txpatch(ii,part).lbph{ep}	= data_e(3:end,rap(k)); % only lpb hist.
                        max_txpatch(ii,part).numel{ep}	= dim_cluster(k);
                        ep = ep + 1;
                    end
                end
			...
			clear rap data_e epitome dim_cluster
		end
		ii = ii+1;
